...
export default function OnboardingForm({ onComplete, user }) {
	...
	const handleSkipForLater = async () => {
		...
		try {
			if (onComplete) {
				// Call parent's onComplete with profileData and skipBaseline flag
				await onComplete(form, true);
			}
			setLoading(false);
		} catch (err) {
			setLoading(false);
	...
	const submit = async () => {
		...
		try {
			if (onComplete) {
				// Call parent's onComplete with profileData and skipBaseline=false
				await onComplete(form, false);
			}
			setLoading(false);
		} catch (err) {
			setLoading(false);
						...
						<div>
							...
							<div className="grid grid-cols-2 md:grid-cols-3 gap-3">
								{equipmentOptions.map((eq) => {
									const active = form.equipment.includes(
										eq.key
									);
				...
				return (
					<div className="space-y-6">
						...
						<p className="text-base-content/70 mb-4">
							You can add your baseline metrics now or update them
							later.
						</p>
						...
						<div className="grid grid-cols-2 gap-4">
							...
							<div>
								...
								<input
									...
									max="300"
									value={
										form.baseline_metrics.height_cm || ""
									}
									onChange={(e) =>
										setForm({
											...form,
							...
							<div>
								...
								<input
									...
									max="500"
									value={
										form.baseline_metrics.weight_kg || ""
									}
									onChange={(e) =>
										setForm({
											...form,
