...
const PlanTemplateDetail = () => {
	...
	const { currentTemplate, isLoading, getPlanTemplateById } =
		usePlanTemplateStore();
	const { assignPlanToUser, isLoading: assignLoading } = useUserPlanStore();
	...
	const handleSelectPlan = async () => {
		const result = await assignPlanToUser(id);
		if (result.success) {
			toast.success("Plan selected successfully! 🎉");
			setShowConfirmModal(false);
			navigate("/dashboard");
		} else {
			toast.error(result.message || "Failed to select plan");
		}
	};
				...
				<div className="card-body">
					...
					<div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
						...
						<div className="stat bg-base-200 rounded-lg p-4">
							...
							<div className="stat-value text-2xl">
								{currentTemplate.dayTemplates?.reduce(
									(sum, day) =>
				...
				<dialog className="modal modal-open">
					<div className="modal-box">
						...
						<p className="text-sm text-base-content/70 mb-6">
							This will replace your current workout plan if you
							have one active.
						</p>
